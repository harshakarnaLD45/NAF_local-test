              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: { xs: 1, sm: 2, md: 3 }, transition: 'transform 1s ease-in-out' }}>
                ...
                {isMobile && (
                  <Box>
                    ...
                    <Box
                      className={`menu-item ${(strippedPath === '/membership') ? 'bodyMediumText1' : 'bodyRegularText2'}`}
                      sx={{
                        display: 'flex',
                        alignItems: 'center',
                        justifyContent: 'flex-start',
                        mt: '10px',
                        color: (strippedPath === '/membership') ? '#1A1A1A' : '#FCFCFC',
                        gap: '10px',
                      }}
                      onClick={() => handleNavigation(`/${currentLang}/membership`)}
                    >
                      {(strippedPath === '/membership') && <span className="arrow-icon"><ArrowIcon1 /></span>}
                      <span>{t("memebersLogin.MembershipTitle")}</span>
                    </Box>
                    {/* Contact Page Menu */}
                    <Box
...

        {
          !isMobile && (
            <Box sx={{ display: 'flex', alignItems: 'center', marginTop: '16px', gap: { xs: '4px', sm: '6px', md: '10px' } }}>
              <Box>
                <button data-cursor="hover" className='profile-button' onClick={() => navigate(`/${currentLang}/membership`)}>
                  <ProfileIcon />
                </button>
